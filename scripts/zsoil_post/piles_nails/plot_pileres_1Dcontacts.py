...
# get bounds:
    ...
    for k in range(len(pilecrds)):
        ...
        if mat==15: # pile on symmetry axis --> Fn*2
            ...
            FN_stutzen_max = min(FN_stutzen_max,FN[k][0]*2)
        elif pos[1] < 1:
...
##            qratio += np.mean([step.cnt.str_level[ke][kgp] for kgp in [0,1]])*dl
    ...
    p[0][1].set_alpha(0.4)
    ax.annotate('Normalkraft Pfahlkopf (deja x2 sur l\'axe de symmetrie) \n Min Fn - Stutzen %1.0f kN - Feld %1.0f kN'%(FN_stutzen_max,FN_feld_max),xy=(34,-3.5),va='center',size=8)
